  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Feriados & Datas Importantes</h1>
          <p className="text-xs sm:text-sm text-gray-500">Calendário usado para validar folgas e escalas</p>
        </div>
      </div>
        ...
        <form onSubmit={handleAdd} className="p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="recorrente" className="text-sm text-gray-600">Recorrente (todo ano)</label>
            </div>
                ...
                <div key={f.id} className="px-4 py-3 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className="font-mono text-xs text-gray-600">{fmtDateShort(f.data, f.recorrente)}</span>
                      <Badge variant={f.tipo === "NACIONAL" ? "destructive" : f.tipo === "EMPRESA" ? "warning" : "secondary"}>
                        {TIPO_LABEL[f.tipo] ?? f.tipo}
                      </Badge>
                      {f.recorrente
                        ? <span className="text-xs text-green-600">Todo ano</span>
                        : <span className="text-xs text-gray-400">{new Date(f.data).getUTCFullYear()}</span>}
                    </div>
                  </div>
                  <button
                    onClick={() => handleDelete(f.id)}
                    className="p-2 text-gray-300 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors shrink-0"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
